    ...
    <>
      ...
      {isCollapsed && (
        <Button
          ...
          className="fixed top-4 left-4 z-[100] h-12 w-12 bg-card/80 backdrop-blur-lg border-border/50 shadow-pastel-lg hover:shadow-pastel hover:scale-105 transition-all duration-300 group"
        >
      ...
      <Sidebar 
        ref={sidebarRef}
        className="border-r-0 bg-sidebar shadow-pastel-lg z-50 [&[data-mobile=true]]:bg-sidebar"
      >
        <SidebarHeader className="border-b border-sidebar-border p-6 bg-gradient-to-r from-sidebar to-pastel-blue-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-primary via-primary-light to-accent shadow-pastel">
              ...
              {!isCollapsed && (
                <div className="flex flex-col animate-fade-in">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="secondary" className="bg-accent/20 text-accent border-accent/30 text-xs">Teacher</Badge>
                  </div>
                  <span className="text-xs text-sidebar-foreground/70">Teacher Portal</span>
                </div>
              )}
            </div>
        ...
        <SidebarContent className="px-4 py-6 bg-gradient-to-b from-sidebar to-pastel-blue-50">
          <SidebarGroup>
            <SidebarGroupLabel className="text-sidebar-foreground/60 font-semibold mb-4 px-2 flex items-center gap-2">
              {!isCollapsed && (
                <>
                  ...
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton 
                      ...
                      className={`
                        ...
                        ${isActive(item.url) 
                          ? 'bg-gradient-to-r from-primary/20 to-accent/20 text-primary border-primary/30 shadow-pastel font-semibold' 
                          : 'text-sidebar-foreground/80 hover:text-primary hover:border-border/50 hover:bg-sidebar-accent/30'
                        }
                      `}
                            ...
                            <div className="flex items-center justify-between w-full">
                              ...
                              <div className="flex items-center gap-2">
                                {item.badge && (
                                  <Badge 
                                    variant="secondary" 
                                    className="bg-accent/20 text-accent border-accent/30 text-xs h-5 min-w-5 flex items-center justify-center font-semibold"
                                  >
                                    {item.badge}
                                  </Badge>
                                )}
